...
func _physics_process(delta: float) -> void:
	...
	#if buffer_manager_c.buffer_d.size() > 0:
		#self.position = buffer_manager_c.remove_from_buffer()["position"]
	
	#if input_manager.input_buffer.size() > 0 and connection_manager_c.connected:
		#packet_id += 1
		#var input_vec : Vector2 = input_manager.input_buffer.pop_front()
		#var input_dict : Dictionary = {"player_id" : connection_manager_c.player_id, "input_vec" : input_vec, "packet_id": packet_id}
...
#func server_reconciliation(delta : float) -> void:
	...
	#var update_packet : Dictionary
		...
		#
		#var start : int = update_packet["packet_id"] - 1
